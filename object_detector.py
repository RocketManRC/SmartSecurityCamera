# Added lines 128 and 129 to deal with metadata that has b'???' as the first entry. RM.
    ...
    try:
      with zipfile.ZipFile(model_path) as model_with_metadata:
        ...
        with model_with_metadata.open(file_name) as label_file:
          ...
          if label_list[0] == b'???': # it seems the list may have b'???' as the first entry so remove it if so
              del label_list[0]
